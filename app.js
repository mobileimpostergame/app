...
const SYSTEM_PROMPT = [
  ...
  "Category must be broad but specific enough for gameplay.",
  "Secret word must be common enough for most people to understand and discuss.",
...
function renderHome() {
  ...
  return `
    <section class="screen with-bottom-bar">
      ...
      <div class="bottom-bar">
        <button class="button" type="button" data-action="start-game">Start Game</button>
      </div>
    </section>
  `;
}
...
function renderRandomPanel() {
  ...
  return `
    <div class="count-row">
      <div>
        <h3>Random imposters</h3>
        <p>Each round chooses a hidden number from 1 to ${max}.</p>
      </div>
...
function renderRangePanel() {
  return `
    ${renderCountStepper("rangeMin", "Minimum", "Lowest possible imposters.")}
...
function renderChaosPanel() {
  return `
    <div class="count-row">
      <div>
        <h3>Joke mode</h3>
        <p>Could be 0. Could be everyone. No one gets to relax.</p>
      </div>
      <span class="step-number">0-${state.players.length}</span>
    </div>
  `;
}
...
function renderReveal() {
  ...
  return `
    <section class="screen center-screen ${showGotIt ? "with-bottom-bar" : ""}">
      <header class="app-header">
        <button class="nav-button" type="button" aria-label="Back" data-action="back">${icon("back")}</button>
        <span class="nav-spacer"></span>
      </header>
      ...
      <div class="reveal-layout">
        ...
        <div class="reveal-center">
          ...
          <button class="reveal-card ${revealedClass}" type="button" data-action="${state.revealOpened ? "" : "open-reveal"}" ${state.revealOpened ? "disabled" : ""} aria-label="Tap the box to reveal">
            ${state.revealOpened ? "" : sparkleField(92)}
      ...
      ${showGotIt ? `<div class="bottom-bar"><button class="button" type="button" data-action="got-it">Got it!</button></div>` : ""}
    </section>
  `;
}
  ...
  return `
    <section class="screen with-bottom-bar">
      ...
      <div class="final-card-list">
        ...
        ${renderPhaseCard("team", "Who is staring?", "Listen for the player who sounds almost right.", "purple", 2)}
      ...
      <div class="bottom-bar">
        <button class="button danger" type="button" data-action="reveal-results">Reveal Results</button>
      </div>
    </section>
  `;
}
...
function clampImposterSettings() {
  ...
  state.settings.rangeMin = clamp(Number(state.settings.rangeMin) || 1, 1, maxNormal);
  state.settings.rangeMax = clamp(Number(state.settings.rangeMax) || state.settings.rangeMin, 1, maxNormal);
  ...
  if (!["fixed", "random", "range", "chaos"].includes(state.settings.imposterMode)) {
...
function getImposterSummary() {
  ...
  const maxNormal = Math.max(1, state.players.length - 1);
  ...
  if (settings.imposterMode === "random") {
    return {
      short: "?",
      detail: `A hidden random count from 1 to ${maxNormal}.`
    };
  }
  ...
  return {
    short: "0-All",
    detail: `Joke mode can choose anywhere from 0 to ${state.players.length}.`
  };
}
...
async function fetchGroqRound(attempt, imposterCount) {
  ...
  const userPrompt = [
    ...
    "Examples of possible categories include movie, animal, location, food, sports, brands, and jobs, but do not restrict yourself to those.",
...
function resolveImposterCount() {
  ...
  if (settings.imposterMode === "random") return randomInt(1, maxNormal);
