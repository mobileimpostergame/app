...
const RECENT_ROUNDS_KEY = "the-imposter-game-recent-rounds-v1";
const MIN_PLAYERS = 1;
...
const VARIETY_LANES = [
  "food and drink people actually order, snack on, or argue about",
  "animals, pets, zoo animals, and funny creatures normal people know",
  "places, travel, holidays, buildings, and everyday locations",
  "movies, TV, streaming, celebrities, memes, and pop culture",
  "music, artists, songs, concerts, festivals, and nightlife",
  "sports, gym, playground games, and casual competitive activities",
  "brands, shops, apps, tech people use every day, and social media",
  "jobs, school, office life, hobbies, and everyday routines",
  "fashion, beauty, accessories, grooming, and things people wear",
  "home objects, rooms, furniture, chores, and everyday possessions",
  "toys, games, parties, childhood things, and weekend activities",
  "weather, outdoors, nature, beaches, parks, and seasonal stuff",
  "transport, city life, roads, vehicles, and public places",
  "emotions, habits, social situations, and relatable human moments"
];
...
const defaultSettings = {
  ...
  teammateKnowledgeMode: "all",
  knownTeammateCount: 1
};
...
app.addEventListener("click", (event) => {
  ...
  switch (action) {
    ...
    case "set-imposter-mode":
      ...
      break;
    case "set-knowledge-mode":
      setKnowledgeMode(target.dataset.mode);
      break;
    case "step":
...
function setScreen(screen, options = {}) {
  ...
  requestAnimationFrame(() => {
    scrollPageTo(Number.isFinite(options.scrollY) ? options.scrollY : 0, "auto");
  });
}

function getPageScrollY() {
  return window.scrollY || document.documentElement.scrollTop || document.body.scrollTop || app.scrollTop || 0;
}

function scrollPageTo(top, behavior = "auto") {
  const scrollOptions = { top: Math.max(0, top), behavior };
  ...
  document.body.scrollTop = scrollOptions.top;
}

function scrollPageToBottom(behavior = "auto") {
  const target = Math.max(
    document.documentElement.scrollHeight,
    document.body.scrollHeight,
    app.scrollHeight
  );
  scrollPageTo(target, behavior);
  window.setTimeout(() => scrollPageTo(target, "auto"), 80);
}
...
function goBack() {
  ...
  if (state.screen === "reveal") {
    if (state.revealOpened) {
      ...
      setScreen("players", { scrollY: state.playersScrollY });
    }
    return;
  }
  ...
  return `
    <section class="screen home-screen">
      ...
      <div class="settings-card">
        ...
        ${state.settings.impostersKnowEachOther ? renderTeammateKnowledgePanel() : ""}
      </div>
...
function renderTeammateKnowledgePanel() {
  const mode = state.settings.teammateKnowledgeMode;
  const summary = getTeammateKnowledgeSummary();
  return `
    <div class="settings-subpanel">
      <div class="subpanel-heading">
        <div>
          <h3>Known Teammates</h3>
          <p>${escapeHtml(summary)}</p>
        </div>
      </div>
      <div class="mini-segmented">
        ${renderKnowledgeSegment("all", "All")}
        ${renderKnowledgeSegment("fixed", "Number")}
        ${renderKnowledgeSegment("random", "Random")}
      </div>
      ${mode === "fixed" ? `
        <div class="compact-step-row">
          <div>
            <h4>Show teammates</h4>
            <p>Each imposter sees this many other imposters.</p>
          </div>
          ${renderStepperControls("knownTeammateCount")}
        </div>
      ` : ""}
    </div>
  `;
...
function renderKnowledgeSegment(mode, label) {
  const active = state.settings.teammateKnowledgeMode === mode;
  return `
    <button class="mini-segment ${active ? "active" : ""}" type="button" data-action="set-knowledge-mode" data-mode="${mode}">
      ${escapeHtml(label)}
    </button>
  `;
}
...
function renderCountStepper(setting, title, text) {
  return `
    <div class="count-row">
      ...
      ${renderStepperControls(setting)}
    </div>
  `;
}

function renderStepperControls(setting) {
  const value = state.settings[setting];
  const bounds = getSettingBounds(setting);
  const minusDisabled = value <= bounds.min;
  const plusDisabled = value >= bounds.max;
  return `
    <div class="stepper">
      <button class="step-button" type="button" data-action="step" data-setting="${setting}" data-delta="-1" aria-label="Decrease" ${minusDisabled ? "disabled" : ""}>${icon("minus")}</button>
      <span class="step-number">${value}</span>
      <button class="step-button" type="button" data-action="step" data-setting="${setting}" data-delta="1" aria-label="Increase" ${plusDisabled ? "disabled" : ""}>${icon("plus")}</button>
    </div>
  `;
}
...
function renderRangePanel() {
  return `
    ...
    ${renderCountStepper("rangeMax", "Maximum", "Can go all the way up to everyone.")}
  `;
}
...
function renderImposterBubbles(player) {
  ...
  if (state.settings.impostersKnowEachOther) {
    const visibleTeammateIds = state.round.knownTeammatesByPlayer?.[player.id] || [];
    const teammates = visibleTeammateIds
...
function addPlayer() {
  ...
  saveSettings();
  render();
  requestAnimationFrame(() => scrollPageToBottom("smooth"));
}
...
function setKnowledgeMode(mode) {
  state.settings.teammateKnowledgeMode = mode;
  clampImposterSettings();
  saveSettings();
  render();
}
...
function clampImposterSettings() {
  ...
  const maxImposters = Math.max(1, playerCount);
  const maxTeammates = Math.max(1, playerCount - 1);
  if (state.settings.imposterMode === "random") state.settings.imposterMode = "chaos";
  state.settings.fixedCount = clamp(Number(state.settings.fixedCount) || 1, 1, maxImposters);
  state.settings.rangeMin = clamp(toInteger(state.settings.rangeMin, 0), 0, maxImposters);
  state.settings.rangeMax = clamp(toInteger(state.settings.rangeMax, Math.max(1, state.settings.rangeMin)), 0, maxImposters);
  ...
  if (!["all", "fixed", "random"].includes(state.settings.teammateKnowledgeMode)) {
    state.settings.teammateKnowledgeMode = "all";
  }
  state.settings.knownTeammateCount = clamp(toInteger(state.settings.knownTeammateCount, 1), 1, maxTeammates);
}

function getSettingBounds(setting) {
  const playerCount = state.players.length;
  const maxImposters = Math.max(1, playerCount);
  const maxTeammates = Math.max(1, playerCount - 1);
  const bounds = {
    fixedCount: { min: 1, max: maxImposters },
    rangeMin: { min: 0, max: maxImposters },
    rangeMax: { min: 0, max: maxImposters },
    knownTeammateCount: { min: 1, max: maxTeammates }
  };
  return bounds[setting] || { min: 0, max: 100 };
}
...
function getTeammateKnowledgeSummary() {
  const settings = state.settings;
  if (settings.teammateKnowledgeMode === "random") {
    return "Each imposter sees a random number from 1 to all available teammates.";
  }
  if (settings.teammateKnowledgeMode === "fixed") {
    const count = settings.knownTeammateCount;
    return `Each imposter sees ${count} teammate${count === 1 ? "" : "s"}.`;
  }
  return "Each imposter sees all other imposters by default.";
}
...
async function fetchGroqRound(attempt, imposterCount) {
  ...
  const recentRounds = getRecentRounds();
  const recentCategories = uniqueText(recentRounds.map((round) => round.category)).slice(0, 10);
  const recentClues = uniqueText(recentRounds.map((round) => round.clue)).slice(0, 10);
  const varietyLane = pickVarietyLane(recentRounds);
  const userPrompt = [
    ...
    `This round must use this variety lane: ${varietyLane}.`,
    "Choose a secret word that feels clearly different from the recent words and categories below.",
    "Do not pick another word from the same obvious cluster if recent words are similar, for example avoid more fast food after pizza/burger/taco or more pets after dog/cat/hamster.",
    "Good broad category styles include snack, fast food, animal, movie, TV show, pop song, celebrity, app, holiday, place, sport, brand, job, drink, toy, game, hobby, everyday object, fashion, travel, or party.",
    ...
    `Avoid these recent categories where possible: ${recentCategories.length ? recentCategories.join(", ") : "none"}.`,
    `Avoid these recent imposter clues where possible: ${recentClues.length ? recentClues.join(", ") : "none"}.`,
  ...
  rememberRound(normalized, varietyLane);
  return normalized;
}
...
function createRound(content, imposterCount) {
  ...
  const knownTeammatesByPlayer = createKnownTeammatesByPlayer(imposterIds);
  state.round = {
    id: createId(),
...
function resolveImposterCount() {
  ...
  const maxImposters = Math.max(1, playerCount);
  const settings = state.settings;
  if (settings.imposterMode === "fixed") return clamp(settings.fixedCount, 1, maxImposters);
  if (settings.imposterMode === "range") return randomInt(settings.rangeMin, settings.rangeMax);
  return randomInt(0, playerCount);
}

function createKnownTeammatesByPlayer(imposterIds) {
  const map = {};
  if (!state.settings.impostersKnowEachOther) return map;

  imposterIds.forEach((imposterId) => {
    const teammates = shuffle(imposterIds.filter((id) => id !== imposterId));
    if (!teammates.length) {
      map[imposterId] = [];
      return;
    }

    let visibleCount = teammates.length;
    if (state.settings.teammateKnowledgeMode === "fixed") {
      visibleCount = clamp(state.settings.knownTeammateCount, 1, teammates.length);
    }
    if (state.settings.teammateKnowledgeMode === "random") {
      visibleCount = randomInt(1, teammates.length);
    }
    map[imposterId] = teammates.slice(0, visibleCount);
  });
...
function openPlayerReveal(id) {
  ...
  state.playersScrollY = getPageScrollY();
...
function getRecentRounds() {
  try {
    const value = JSON.parse(localStorage.getItem(RECENT_ROUNDS_KEY) || "[]");
    if (!Array.isArray(value)) return [];
    return value
      .filter((item) => item && typeof item === "object")
      .map((item) => ({
        word: cleanContentValue(item.word, 42),
        category: cleanContentValue(item.category, 32),
        clue: cleanContentValue(item.clue, 28),
        lane: cleanContentValue(item.lane, 90)
      }))
...
function rememberRound(content, lane) {
  const recentWords = getRecentWords().filter((item) => item.toLowerCase() !== content.secretWord.toLowerCase());
  recentWords.unshift(content.secretWord);
  localStorage.setItem(RECENT_WORDS_KEY, JSON.stringify(recentWords.slice(0, 24)));

  const recentRounds = getRecentRounds().filter((item) => item.word.toLowerCase() !== content.secretWord.toLowerCase());
  recentRounds.unshift({
    word: content.secretWord,
    category: content.category,
    clue: content.imposterClue,
    lane
  });
  localStorage.setItem(RECENT_ROUNDS_KEY, JSON.stringify(recentRounds.slice(0, 24)));
}

function pickVarietyLane(recentRounds) {
  const recentLanes = new Set(recentRounds.slice(0, 6).map((round) => round.lane).filter(Boolean));
  const freshLanes = VARIETY_LANES.filter((lane) => !recentLanes.has(lane));
  const pool = freshLanes.length ? freshLanes : VARIETY_LANES;
...
function uniqueText(items) {
  const seen = new Set();
  return items
    .map((item) => cleanContentValue(item, 48))
    .filter(Boolean)
    .filter((item) => {
